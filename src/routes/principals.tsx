...
function principalsPage() {
  ...
  const filtered = items.filter((p) => {
    ...
    if (q && !`${p.company} ${p.contact} ${p.country}`.toLowerCase().includes(q.toLowerCase()))
      return false;
    return true;
  });
  ...
  const addMutation = useMutation({
    mutationFn: (p: Omit<principal, "id">) =>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-3 md:grid-cols-4">
        {[
          { label: "Total Clients", value: items.length, sub: "across 6 regions" },
            ...
            <tbody>
              {filtered.map((p) => (
                <tr key={p.id} className="border-b last:border-0 hover:bg-muted/30">
                  ...
                  <td>
                    <span
                      className={`inline-flex h-6 w-6 items-center justify-center rounded-full border text-xs font-bold ${priorityColor[p.priority]}`}
                    >
                      {p.priority}
                    </span>
                  </td>
                  <td>
                    <span
                      className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${statusColor[p.status]}`}
                    >
                      {p.status}
                    </span>
                  </td>
      ...
      <div className="grid gap-3 py-2">
        ...
        <div className="grid grid-cols-3 gap-3">
          <Field label="Tier">
            <Select
              value={form.priority}
              onValueChange={(v) => setForm({ ...form, priority: v as principal["priority"] })}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
